...
- Read `PROFILE.md` and any packet `artifact_paths.profile_context` before council review. Treat it as the user's current risk profile unless the user provides newer explicit limits.
...
6. Record confidence, dissent, missing evidence, invalidated assumptions, profile_fit, and no-action triggers for each horizon.
7. If `PROFILE.md` covers the relevant sizing/risk basis, do not use "risk limits unspecified" as a no-action reason. Decide no action only when evidence fails, profile gates fail, artifacts are missing, consensus is weak, or horizon conflicts are unresolved.
