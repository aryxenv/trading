...
11. Require web/news evidence, historical trend checks, top-down analysis, statistical grounding, bias stripping, and pruned dead ends for each horizon.
12. Consolidate only sourced, factual findings. Treat missing IBKR API headlines as neutral, not bearish. Preserve conflicts between horizons instead of averaging them away.
13. Write the consolidated research packet to `sandbox/<run-id>/research-packet.json`.
14. Invoke `council-orchestrator` through the `agent` tool with the research packet path.
15. Return the final report path from `council-orchestrator`.
