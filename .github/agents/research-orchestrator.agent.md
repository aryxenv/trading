...
- Include `PROFILE.md` as `artifact_paths.profile_context`. If it is missing or older than the portfolio snapshot, derive/update it from `portfolio.json` before research packet validation. Exclude account id.
...
5. Read `PROFILE.md`; if missing or stale, update it from the portfolio snapshot with NAV, cash %, concentration, exposure clusters, sizing bands, and speculative/cash gates.
6. Run `uv run python -m ibkr.scripts.position_context --target <resolved-symbol> --snapshot sandbox/<run-id>/portfolio.json --output sandbox/<run-id>/position-context.json`.
7. Run `uv run python -m ibkr.scripts.target_context --target <resolved-symbol> --snapshot sandbox/<run-id>/portfolio.json --output sandbox/<run-id>/target-context.json`.
8. Run `uv run python -m ibkr.scripts.ibkr_news --target <resolved-symbol> --output sandbox/<run-id>/ibkr-news.json`.
9. If the target is absent, treat it as a new investment using available cash plus portfolio shift candidates and `PROFILE.md` sizing gates.
10. Spawn independent `market-research-agent` runs. At minimum use horizon-specific fleet routes:
...
11. Give each market agent a unique sandbox folder, `ibkr-news.json`, `PROFILE.md`, the assigned horizon, and a clear research route. Add extra cross-cutting agents only when useful, but their output must still map findings back to the three horizons.
12. Require each route to write `findings.json` with target, run id, horizon, sources, facts, stats, contrarian evidence, pruned routes, missing evidence, no-action triggers, confidence, profile_fit, and cited source/fact ids.
13. Require web/news evidence, historical trend checks, top-down analysis, statistical grounding, bias stripping, and pruned dead ends for each horizon.
14. Consolidate only sourced, factual findings. Treat missing IBKR API headlines as neutral, not bearish. Preserve conflicts between horizons instead of averaging them away.
15. Write the consolidated research packet to `sandbox/<run-id>/research-packet.json`.
16. Run `uv run python -m ibkr.scripts.validate_research_packet --input sandbox/<run-id>/research-packet.json`.
17. Invoke `council-orchestrator` through the `agent` tool with the research packet path only if validation has no structural errors.
18. Return the final report path from `council-orchestrator`.
